...
async def main():
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Use natural language to perform search
            search_result = await session.browser.agent.act_async(page, ActOptions(
                action="Type 'AGB browser automation' in the search box and press Enter",
                timeoutMS=15000
            ))
            
            print(f"  Search action result: {search_result.success}")
            ...
            click_result = await session.browser.agent.act_async(page, ActOptions(
                action="Click on the first search result link",
                timeoutMS=10000
            ))
