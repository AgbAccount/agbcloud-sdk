...
class SessionManager:
    ...
    def create_session(self, name: str, labels: dict = None):
        """Create a named session with optional labels"""
        params = CreateSessionParams(labels=labels or {})
...
def create_session_with_task(agb: AGB, task_config: Dict[str, Any]):
    ...
    params = CreateSessionParams(labels=task_config.get("labels", {}))
    result = agb.create(params)
...
agb = AGB()
tasks = [
    {"code": "print('Task 1')", "labels": {"task": "1"}},
    {"code": "print('Task 2')", "labels": {"task": "2"}},
    {"code": "print('Task 3')", "labels": {"task": "3"}},
]
...
```python
# Organize sessions with meaningful labels
...
- **[API Reference](../api-reference/python/session.md)** - Complete session API documentation